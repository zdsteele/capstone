...
yfinance daily bars
        ...
        ▼
Bronze Delta  bootcamp_students.zdsteele.bronze_*   + Volume bronze_edgar_raw
...
2. Run notebooks `01`→`04` (widgets default to `bootcamp_students` / `zdsteele`).
   `05` is optional (keyword search by default).
3. Set up **reverse** CDC only: the 6 `bootcamp_students.edgar_zdsteele_*` tables →
   `bootcamp_students.zdsteele.lb_*_history`. Forward reads go through the
