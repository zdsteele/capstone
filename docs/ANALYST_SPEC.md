...
| 6 | Balance sheet health | ✅ | `gold_financial_ratios`: cash + marketable securities, short-term + long-term debt + leases → `total_debt` / `net_debt_full`, `current_ratio`, `debt_to_ebitda`, `net_debt_to_ebitda`, `interest_coverage` (EBIT ÷ interest expense), `goodwill_pct_assets`, with trend flags. Upcoming-maturity schedule still not parsed (needs the debt footnote). |
| 7 | Capital allocation | ✅ | `gold_financial_ratios`: dividends paid, buybacks, acquisitions (abs), `dividend_payout`, `fcf_payout`, `buyback_pct_fcf`, `shareholder_cash_return`; `gold_valuation`: `dividend_yield`, `buyback_yield`, `shareholder_yield`. |
...
| 20 | Investor Health Score (0–100) | ✅ | `gold_company_health` (nb 10): **11 dimensions** — growth quality, profitability, cash generation, balance sheet, capital allocation, capital efficiency, **management/governance, accounting quality, valuation, sector-specific**, financial health (composite) — plus overall score + label + direction. `ai_query` is grounded on the full §3-10 ratio set + `gold_valuation` + `gold_governance` + `gold_insider_activity` + the filing briefings. A dimension whose input block is absent returns `null` (not faked). Composite must not hide a flagged risk. |
