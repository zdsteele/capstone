...
| 18 | Valuation | ◑ | `gold_valuation` (nb 11): latest yfinance close × diluted shares → market cap, EV, P/E, EV/EBIT, EV/Revenue, Price/FCF, FCF yield, Price/Book. Flows use the latest FY (annualised from 4 quarters if no FY row). No forward P/E, no dividend yield (dividends-paid not ingested), no peer/historical-range comparison yet. Agent `get_valuation` tool + Dashboard panel. COMPANY QUALITY (nb 10) stays strictly separate. |
